...
export default async function EventDetailsContent({
    ...
    if (!row) {
        logger.warn("Event not found or user lacks ownership", "EventDetailUI", { eventId, userId });
        notFound();
    }

    logger.debug("Rendered EventDetailsContent", "EventDetailUI", { eventId, userId, rsvpCount: row.rsvps.length });
